...
for(i in 1:200){
  ...
  gamma[i] <- Gamma(FUN = dnt1, epsilon = 0.0001, S = x[i], K = 1e6,
                    U = 0.96, L = 0.92, sigma = 0.06, T = 0.5, r = 0.02, b = -0.02) 
}
plot(x, vega, type = "l", xlab = "S",ylab = "", main = "Vega")
...
d <- read.table("audusd.csv", colClasses = c("character", rep("numeric", 5)),
                sep = ";",header = TRUE)
...
for (i in 1:n) {
  option_price[i] <- dnt1(S = underlying[i], K = 1000000, U = 0.96,
                          L = 0.92, sigma = 0.06, T = t[i]/(60*24*365), r = 0.0025,
                          b = -0.0250)
}
a <- min(option_price)
b <- max(option_price)
